...
class HrLoanAcc(models.Model):
    ...
    employee_account_id = fields.Many2one('account.account', string="Payment Account")
    treasury_account_id = fields.Many2one('account.account', string="Expense Account")
...
class HrLoanLineAcc(models.Model):
    ...
    def action_paid_amount(self):
        ...
        for line in self:
            ...
            move = self.env['account.move'].create(vals)
            journal = vals.get('journal_id') and self.env['account.journal'].browse(vals.get('journal_id'))
            move.post()
            print('#################',journal)
        return True
