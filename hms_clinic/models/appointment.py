...
class Appointment(models.Model):
    ...
    _description = 'request appointment for a doctor in clinic'
    _order = "request_time desc"

    name = fields.Char(string="appointment #")
    patient = fields.Char("Patient",required=True)
    clinic = fields.Many2one("hms.clinic",string="clinic",required=True)
    doctor = fields.Many2one("hms.doctor",domain="[('clinic', '=', clinic)]",string="Doctor",required=True)
    fees = fields.Float(related="doctor.fees")
    ...
    def create(self, vals):
        vals['name'] = self.env['ir.sequence'].next_by_code('hms.appointment') or _('New')
    ...
    def create_invoice(self):
        ...
        invoice_line = []
        item = self.env['product.template'].search([("name","=",self.doctor.name)],limit=1) 
        item_id = item.id
